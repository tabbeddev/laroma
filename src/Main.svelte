...
{#if appState === State.Home}
  ...
  <SlideInBox>
    <SecondButton
      title={nextStudy[0]}
      Icon={nextStudy[1] ? TriangleAlert : undefined}
      onclick={() => {
        if (nextStudy[1]) {
    ...
    {:else}
      <div class="centered">
        ...
        {#if sets.invalid_urls.length > 0}
          <p>
            <strong>Hinweis:&nbsp;</strong>{sets.invalid_urls.length === 0
              ? "Ein Lernset ist "
              : "Mehere Lernsets sind "}nicht mehr verfügbar.
          </p>
        {/if}
      </div>
    {/if}
...
{:else if appState === State.ChangeStudySet}
  <SlideInBox>
    ...
    />
    <SecondButton
      title="Lernset Creator"
      onclick={() => {
        appState = State.Creator;
      }}
      Icon={Pencil}
    />
  </SlideInBox>
